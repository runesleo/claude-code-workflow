...
Phase 1 made the adapter contract explicit. Phase 2-3 added a minimal `bin/vibe` generator with `inspect` and `switch`. Phase 4 adds portable behavior-policy rendering plus deeper native permission or config outputs where the host supports them.
...
4. If a host lacks a native skill system, degrade to rules, AGENTS guidance, or wrapper commands instead of inventing fake parity.
